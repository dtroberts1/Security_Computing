...
unsigned int calcBits(fileContents *inputFileContents, int checkSumSize)
{
	...
	int k, l, i, j, n, rem, incrementer, a;
	long unsigned int appendNbr; 
	switch (checkSumSize)
	{
		case 16:
			i = 0, j = 1, incrementer = 2, a = 16, k = i, l = j;
			break;
		case 32:
			k = 0, l = 1, i = 2, j = 3, incrementer = 4, a = 0;
	}
	...
	/* already assigned this at beginning. 
	if (checkSumSize == 16)
		k = i;
	*/
	if (rem > 0) // Remainder of 1, 2, or 3 (or just remainder of 1 if not 32-bit)
	{
		...
		printf("\nj : %d", j); // uses X
			...
			if (rem > 1) // Remainder of 2 or 3; this will skip if checkSumSize is not 32
			{
				...
				if (rem > 2) // Remainder of 3. if 8 bit, it will never go inside here. 
				{
							printf("inside rem > 2\n"); fflush(stdout);
					...
					{
						if (getIthBit(inputFileContents->fContents[i], n))	
							setIthBit(&appendNbr, n + 8); // -a not necessary here.
							//	printf("\nAppendnbr is %s", getBits(xOrResult)); printf("\n"); fflush(stdout);
					...
					{
						if (getIthBit('X', n)) 
							setIthBit(&appendNbr, n); // -a not necessary here.
								//	printf("\nAppendnbr is %s", getBits(xOrResult)); printf("\n"); fflush(stdout);
				...
				else // if remainder is == 2, append 2 more 'x's
				{
					for (int z = 1; z >= 0; z--)
					{
						...
						{
							if (getIthBit('X', n)) 
								setIthBit(&appendNbr, n + (8 * (z))); // -a not necessary here.
						}
					}			
				}
			}
			else // if rem is only == 1, just append three more 'x's
			{
				for (int z = 2; z >= 0; z--)
				{
					...
					{
						if (getIthBit('X', n)) 
							setIthBit(&appendNbr, n + (8 * (z)) - a); // if 2nd param of setIthBit is negative, it will just return, so 16 bit will be fine.
