...
char *readAndStrip(FILE *inFile, int *index) // This reads file to an array, and also prints contents of file.
{
	...
	while ((currChar = fgetc(inFile)) != EOF)
	{
		...
		if ((*index) == 512) // Don't allow the number of char's to go beyond 512 (+ null terminator).
			return myArray;		
...
char *createCTArray(char *inPt, char *inKey)
{
	...
	for (i = x = 0; i < 512; i++)
	{
		...
		keyVal = inKey[x] - 'a';
		ptVal = inPt[i] - 'a';
		insertVal = (keyVal + ptVal) % 26; // CHAR AT A + ...............................................
